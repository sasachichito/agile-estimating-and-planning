...
export default class extends iteration {
    ...
    createMainTable(planId, planTitle, scopeId, resourceId, periodStart, periodEnd) {
        ...
        var edit = this.parts.edit(function() {iPlanInstance().editPlan(this.closest('.main-entry'))});
        var trash = this.parts.trash(function() {iPlanInstance().deletePlan(this.closest('.main-entry'))});
    ...
    makePutInput(plan) {
        ...
        this.section.inputBlock1().appendChild(
            this.parts.operationBlock(
                function() {iPlanInstance().erase()},
                function() {iPlanInstance().send()},
            )
        );
    }
